...
def render_svg_chrome(svg_str, out_png="scratch/temp_ablation.png"):
    abs_out = os.path.abspath(out_png)
    html_wrap = "scratch/temp_wrap_ablation.html"
...
with open("mascots/owluko/owluko_master_exact_512.svg", "r", encoding="utf-8") as f:
    master_svg = f.read()

print("1. Baseline (current master):")
res = evaluate(render_svg_chrome(master_svg))
print(f"   IoU: {res['iou']*100:.2f}%, SSIM: {res['ssim']*100:.2f}%, NCC: {res['ncc']*100:.2f}%, Delta E: {res['delta_e']:.2f}")

# Ablation A: Remove forehead brown smudge
svg_a = master_svg.replace('<ellipse cx="300" cy="105" rx="55" ry="32" fill="url(#rBrowShadow)" />', '')
res_a = evaluate(render_svg_chrome(svg_a))
print("2. Remove forehead brown smudge:")
print(f"   IoU: {res_a['iou']*100:.2f}%, SSIM: {res_a['ssim']*100:.2f}%, NCC: {res_a['ncc']*100:.2f}%, Delta E: {res_a['delta_e']:.2f}")

# Ablation B: Also remove diaper oval
svg_b = svg_a.replace('<ellipse cx="256" cy="442" rx="65" ry="18" fill="#523D2C" opacity="0.35" />', '')
res_b = evaluate(render_svg_chrome(svg_b))
print("3. Also remove diaper oval:")
print(f"   IoU: {res_b['iou']*100:.2f}%, SSIM: {res_b['ssim']*100:.2f}%, NCC: {res_b['ncc']*100:.2f}%, Delta E: {res_b['delta_e']:.2f}")
