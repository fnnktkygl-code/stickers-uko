...
ref_feet = ref_full[430:495, 150:360]

def eval_feet_svg(svg_path):
    png = render_svg(svg_path, "scratch/temp_feet_eval.png")
    vec_full = np.array(Image.open(png).convert("RGBA"))
    vec_feet = vec_full[430:495, 150:360]
    
    bg = np.array([11, 15, 23], dtype=np.float32)
    a_ref = ref_feet[:, :, 3:4].astype(float) / 255.0
    r_comp = np.clip(ref_feet[:, :, :3] * a_ref + bg * (1.0 - a_ref), 0, 255).astype(np.uint8)
    
    a_vec = vec_feet[:, :, 3:4].astype(float) / 255.0
    v_comp = np.clip(vec_feet[:, :, :3] * a_vec + bg * (1.0 - a_vec), 0, 255).astype(np.uint8)
    ...
    print(f"FEET REGION: SSIM: {ssim*100:.2f}%, NCC: {ncc*100:.2f}%, Delta E: {delta_e:.2f}")
    return ssim, ncc, delta_e

if __name__ == "__main__":
    eval_feet_svg("scratch/test_owluko_master_v2.svg")
