...
def render_svg_chrome(svg_str, out_png="scratch/temp_owluko_v26.png"):
    abs_out = os.path.abspath(out_png)
    html_wrap = "scratch/temp_wrap_v26.html"
...
svg_content = f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="512" height="512" style="background: transparent;">
  <defs>
    ...
    <radialGradient id="headRightShadow" cx="72%" cy="18%" r="42%">
      <stop offset="0%" stop-color="#7A6250" stop-opacity="0.26" />
    ...
    <!-- Left Amber Iris (Smooth Translucent Amber) -->
    ...
    <!-- Left Porcelain Eyelid Hood (Ivory porcelain down to subtle warm tan lip) -->
    ...
    <!-- Eyelid Cast Shadow on Eyeball (Narrow 2px shadow) -->
  ...
  <g clip-path="url(#bodyClip)">
    ...
    <!-- Lower Belly Shading Gradient (Smooth, NO diaper oval) -->
    <ellipse cx="256" cy="415" rx="90" ry="32" fill="#755F4E" opacity="0.28" />
...
print(f"[V26] IoU: {res['iou']*100:.2f}%, SSIM: {res['ssim']*100:.2f}%, NCC: {res['ncc']*100:.2f}%, Delta E: {res['delta_e']:.2f}, MSE: {res['mse']:.1f}")

with open("scratch/test_owluko_v26.svg", "w", encoding="utf-8") as f:
    f.write(svg_content)
