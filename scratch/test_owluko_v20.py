...
def create_owluko_v20_svg(path="scratch/test_owluko_v20.svg"):
    # Precise silhouette polygon
  ...
  <defs>
    ...
    <!-- Seamless Porcelain Beak Gradient (Starts Y=150 with keylight cream) -->
    <linearGradient id="beakGrad" x1="20%" y1="0%" x2="80%" y2="100%">
      <stop offset="0%" stop-color="#FBE6D1" />
      <stop offset="25%" stop-color="#E8D2BE" />
      <stop offset="65%" stop-color="#B89C84" />
      <stop offset="100%" stop-color="#806550" />
    </linearGradient>
    ...
    <!-- Calibrated Left Foot Gradient matching 3D CGI Knuckle Peaks -->
    <linearGradient id="lFootGrad" x1="50%" y1="0%" x2="50%" y2="100%">
      ...
      <stop offset="85%" stop-color="#B89E88" />
      <stop offset="100%" stop-color="#806550" />
    </linearGradient>

    <!-- Calibrated Right Foot Gradient matching 3D CGI Ambient Tone -->
    <linearGradient id="rFootGrad" x1="50%" y1="0%" x2="50%" y2="100%">
      <stop offset="0%" stop-color="#6E5542" />
      <stop offset="30%" stop-color="#8C725C" />
      <stop offset="65%" stop-color="#A88E78" />
      <stop offset="85%" stop-color="#8E7460" />
      <stop offset="100%" stop-color="#624A37" />
    </linearGradient>
  </defs>
  ...
  <g clip-path="url(#bodyClip)">
    ...
    <!-- Left Foot Soft Crevices (Calibrated falloff, no pitch black) -->
    <path d="M 180 468 C 181 474, 180 480, 179 485" stroke="#7A604A" stroke-width="2.5" stroke-linecap="round" opacity="0.45" fill="none" />
    <path d="M 207 468 C 208 474, 208 481, 208 485" stroke="#664D38" stroke-width="2.5" stroke-linecap="round" opacity="0.55" fill="none" />
    ...
    <path d="M 305 468 C 305 474, 305 481, 305 485" stroke="#523B28" stroke-width="2.5" stroke-linecap="round" opacity="0.55" fill="none" />
    <path d="M 332 468 C 332 474, 332 481, 332 485" stroke="#523B28" stroke-width="2.5" stroke-linecap="round" opacity="0.55" fill="none" />
  </g>

  <!-- 3. Facial Depressions & Shading -->
  <ellipse cx="300" cy="105" rx="55" ry="32" fill="url(#rBrowShadow)" />
  <!-- Left Eye Socket Soft Falloff -->
  <ellipse cx="195" cy="155" rx="38" ry="32" fill="#9C826E" opacity="0.10" />
  <!-- Right Eye Socket Soft Falloff -->
  <ellipse cx="310" cy="155" rx="38" ry="32" fill="#785F4D" opacity="0.20" />
...
if __name__ == "__main__":
    svg_path = create_owluko_v20_svg()
    rendered = render_svg_chrome(svg_path, "scratch/temp_owluko_v20.png")
    res = evaluate(rendered)
    print(f"[V20] IoU: {res['iou']*100:.2f}%, SSIM: {res['ssim']*100:.2f}%, NCC: {res['ncc']*100:.2f}%, Delta E: {res['delta_e']:.2f}, MSE: {res['mse']:.1f}")
