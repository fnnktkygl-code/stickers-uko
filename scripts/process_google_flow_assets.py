...
def process_state(st_id, info):
    ...
    print(f"\n🚀 Traitement de {st_id} ({title})...", flush=True)
    ...
    static_clean = rembg.remove(im_static_sq, session=session, alpha_matting=True, alpha_matting_erode_size=5)
    ...
    for idx, rf in enumerate(raw_frames):
        ...
        cfrm = rembg.remove(frm, session=session, alpha_matting=True, alpha_matting_erode_size=4)
    ...
    print(f"✨ [OK] {st_id} finalisé avec succès !", flush=True)

if __name__ == "__main__":
    for st_id, info in STATE_MAPPING.items():
        process_state(st_id, info)
    print("\n🎉 TOUS LES ÉTATS D'AITUKO SONT TRAITÉS ET INTÉGRÉS !", flush=True)
