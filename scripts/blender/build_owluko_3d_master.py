...
def build_owluko_body(porcelain_mat, json_path="scratch/owluko_contours.json"):
    ...
    # Smooth continuous rings
    body_rings = [r for r in raw_rings if r['y_img'] >= 55.0]
    ...
    # Smooth hemispherical cranial cap
    dome_rings = []
    num_dome = 16
    for d in range(1, num_dome):
        ...
        dome_rings.append({
            'y_img': 38.0 + (d / float(num_dome)) * 17.0,
            'z_3d': z,
            'rx': rx,
            'cx': cx_top,
            'ry': ry,
    ...
    for ring in all_rings:
        ...
        for s in range(num_sectors):
            ...
            # Subtle avian tail flare on posterior lower body (Z in [0.20, 0.70], Y > 0)
            if 0.20 < z < 0.75 and vy > cy:
                # posterior side (back)
                flare_fac = math.sin((z - 0.20) / (0.75 - 0.20) * math.pi)
                vy += 0.055 * flare_fac * math.sin(angle/2.0)**2
...
def build_owluko_eyes(eye_mat, porcelain_mat):
    # Recessed eye placement: Y = -0.520 (radius 0.165, front cornea at Y = -0.685)
    # Perfectly nestled into the skull contour so eyes DO NOT protrude in profile/3-quarter!
    eyes = []
    eye_radius = 0.165
    
    for side, cx in [('Left', -0.305), ('Right', 0.295)]:
        # 1. Eyeball Sphere
        bpy.ops.mesh.primitive_uv_sphere_add(segments=48, ring_count=24, radius=eye_radius, location=(cx, -0.520, 1.450))
        ...
        bpy.ops.mesh.primitive_uv_sphere_add(segments=48, ring_count=24, radius=eye_radius * 1.035, location=(cx, -0.520, 1.450))
        ...
        bpy.ops.mesh.primitive_uv_sphere_add(segments=48, ring_count=24, radius=eye_radius * 1.025, location=(cx, -0.520, 1.450))
        ...
        bpy.ops.mesh.primitive_uv_sphere_add(segments=16, ring_count=8, radius=0.016, location=(cx - 0.035, -0.680, 1.485))
...
def build_owluko_beak(beak_mat):
    ...
    z_mid = (1.470 + 1.180) / 2.0
    depth = 1.470 - 1.180 # 0.290
    bpy.ops.mesh.primitive_cone_add(vertices=32, radius1=0.078, radius2=0.024, depth=depth, location=(0.0, -0.590, z_mid))
...
def build_owluko_wings(porcelain_mat):
    # Teardrop/leaf wings nestled smoothly on the flanks (rounded shoulder, tapering towards tail)
    wings = []
    for side, cx, sgn in [('Left', -0.66, -1.0), ('Right', 0.66, 1.0)]:
        bpy.ops.mesh.primitive_uv_sphere_add(segments=32, ring_count=16, radius=0.20, location=(cx, 0.05, 0.88))
        wing = bpy.context.active_object
        wing.name = f"Wing_{side}"
        wing.scale = (0.42, 1.25, 2.10)
        wing.rotation_euler = (math.radians(14), sgn * math.radians(-8), sgn * math.radians(6))
...
def build_owluko_legs_and_feet(feet_mat):
    ...
    for side, cx in [('Left', -0.275), ('Right', 0.285)]:
        ...
        bpy.ops.mesh.primitive_cylinder_add(vertices=32, radius=0.080, depth=0.22, location=(cx, -0.05, 0.02))
        ...
        # 2. Foot Ankle Base
        bpy.ops.mesh.primitive_uv_sphere_add(segments=24, ring_count=12, radius=0.078, location=(cx, -0.08, -0.08))
        ...
        toes_config = [
            (-20, -0.052, 0.008, 0.170),
            (0,    0.000, 0.000, 0.210),
            (20,   0.052, 0.008, 0.165)
        ]
...
def main():
    ...
    out_png = os.path.join(out_dir, "blender_owluko_master_front.png")
    scene.render.filepath = out_png
    bpy.ops.render.render(write_still=True)
    print(f"Rendered Blender Owluko Master Front: {out_png}")
    
    mascot_png = os.path.abspath("mascots/owluko/owluko_3d_front_render.png")
    scene.render.filepath = mascot_png
    bpy.ops.render.render(write_still=True)
    print(f"Saved mascot render: {mascot_png}")
