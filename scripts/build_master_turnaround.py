#!/usr/bin/env python3
"""
Comprehensive Turnaround Engine for AItuko:
1. Builds exact vector components for all 4 canonical views (Front, 3/4, Profile, Back).
2. Renders the official studio model sheet: aituko_vector_turnaround_sheet.png
3. Generates the standalone pure SVG: mascots/aituko/aituko_master_turnaround.svg
4. Renders the clean Studio 3D reference turnaround sheet for comparison: aituko_studio_turnaround_sheet.png
"""
...
def extract_view_data(view_name):
    ...
    if view_name == "profile" and len(pods_pts) == 0:
        pod_mask = np.zeros_like(char_mask)
        pod_mask[250:378, 94:157] = 255
        pod_mask = (pod_mask > 0) & (char_mask > 0)
        pods_pts.append(get_smooth_contour(pod_mask.astype(np.uint8)*255, eps=1.0))
    elif view_name == "three_quarter" and len(pods_pts) == 0:
        pod_mask = np.zeros_like(char_mask)
...
def render_vector_figure(data, target_h=460, scale=3):
    orig_h = data["h"]
    orig_w = data["w"]
    ...
    cx = canv_w // 2
    cy = int(target_h * scale + 25 * scale)
    rx = int(orig_w * 0.38 * s)
    ry = int(12 * scale)
...
print("Turnaround engine ready.")

def build_turnaround_sheets():
    print("🎨 Rendering 4 canonical vector views...")
    
    # 1. View 1: 0° Face - from our verified flawless spline master
    front_data = extract_view_data("front")
    v1_img = render_vector_figure(front_data, target_h=460, scale=3)
    
    # View 2: 45° 3/4
    tq_data = extract_view_data("three_quarter")
    v2_img = render_vector_figure(tq_data, target_h=460, scale=3)
    
    # View 3: 90° Profile
    prof_data = extract_view_data("profile")
    v3_img = render_vector_figure(prof_data, target_h=460, scale=3)
    
    # View 4: 180° Back
    back_data = extract_view_data("back")
    v4_img = render_vector_figure(back_data, target_h=460, scale=3)
    
    SHEET_W = 1920
    SHEET_H = 820
    
    # --- 1. Vector Turnaround Sheet (Our Model) ---
    ...
    draw_v.text((40, 22), "AITUKO - PLANCHE DE TURNAROUND DU MODELE VECTORIEL MASTER (360 DEGRES)", fill=COLOR_CYAN)
    draw_v.text((40, 52), "Modele de Reference Geometrique Officiel - 4 Vues Orthographiques Calibrees - Porcelaine Blanche Pure et Zero Mains", fill=COLOR_MUTED)
    
    cols = [
        {"angle": "0 DEG", "name": "VUE DE FACE (FRONT)", "desc": "Visor 2 yeux cyan, 2 pods lateraux, 2 pieds V", "img": v1_img},
        {"angle": "45 DEG", "name": "TROIS-QUARTS (3/4)", "desc": "Visor galbe oblique, perspective yeux et pods", "img": v2_img},
        {"angle": "90 DEG", "name": "PROFIL (SIDE)", "desc": "Arc facial convexe, 1 oeil profil, pod centre", "img": v3_img},
        {"angle": "180 DEG", "name": "VUE DE DOS (BACK)", "desc": "Porcelaine pure complete (zero ecran/yeux)", "img": v4_img}
    ]
    ...
    out_vec_brain = os.path.join(BRAIN_DIR, "aituko_vector_turnaround_sheet.png")
    ...
    print(f"✅ Saved vector turnaround sheet: {out_vec_brain}")
    
    # --- 2. Studio 3D Reference Turnaround Sheet (Clean Backdrop) ---
    ...
    studio_crops = ['front', 'three_quarter', 'profile', 'back']
    for i, name in enumerate(studio_crops):
        cx = i * col_w
        ...
        draw_s.text((cx + 38, 130), f"{cols[i]['angle']} - {cols[i]['name']}", fill=COLOR_EMERALD)
        
        # Load cleaned 3D crop
        raw_fig = Image.open(f"scratch/turnaround_views/{name}_clean.png").convert("RGBA")
        rfw, rfh = raw_fig.size
        # scale to target_h = 460
        sc = 460.0 / rfh
        target_w = int(rfw * sc)
        ...
        draw_s.text((cx + 35, SHEET_H - 55), cols[i]["desc"], fill=COLOR_MUTED)
    ...
    print(f"✅ Saved studio turnaround sheet: {out_studio_brain}")
    
    # --- 3. Superposed / Comparative Turnaround Board (Side-by-Side Dual View) ---
    DUAL_H = 1600
    dual_sheet = Image.new("RGBA", (SHEET_W, DUAL_H), COLOR_BG)
    # Paste studio on top
    dual_sheet.paste(sheet_studio, (0, 0))
    # Paste vector on bottom
    dual_sheet.paste(sheet_vec, (0, 780))
    
    out_dual_brain = os.path.join(BRAIN_DIR, "aituko_turnaround_master_board.png")
    dual_sheet.save(out_dual_brain, "PNG")
    dual_sheet.save(os.path.join(WORKSPACE_DIR, "mascots/aituko/aituko_turnaround_master_board.png"), "PNG")
    print(f"✅ Saved dual comparison board: {out_dual_brain}")

if __name__ == "__main__":
    build_turnaround_sheets()
