...
def perfect_matte(pil_img):
    ...
    # Corner labels (background MUST touch borders/corners)
    corner_labels = set([labeled[0,0], labeled[0,-1], labeled[-1,0], labeled[-1,-1], labeled[0, 256], labeled[-1, 256]])
    corner_labels.discard(0)
    
    # True background is ONLY pixels connected to borders
    true_bg = np.isin(labeled, list(corner_labels))
    char_fg = ~true_bg
    char_fg = ndimage.binary_fill_holes(char_fg)
    char_fg = ndimage.binary_closing(char_fg, structure=np.ones((3,3)))
...
def render_and_process(mascot, pose, prompt, master_path):
    ...
    clean_pils = [perfect_matte(Image.open(f)) for f in frame_files]
    ...
    if clean_pils:
        ...
        print(f"✅ {mascot} {pose} finalisé avec succès en {round(time.time() - t0, 1)}s !")
        return True
    return False

if __name__ == "__main__":
    # Render Inuko 10_idea with prominent glowing lightbulb above ears
    inuko_master = f"{BASE_DOWN}/mascots/inuko/master/inuko_greenscreen_master.jpg"
    inuko_idea_prompt = "Starts from the neutral resting stance on solid chroma green (#00FF00). The cute svelte 3D Doberman Inuko perks up excitedly as a prominent glowing 3D yellow glass lightbulb with golden filament hovers brightly above its pointed ears, joyful eureka expression, smooth 3D character animation."
    render_and_process("inuko", "10_idea", inuko_idea_prompt, inuko_master)
    
    # Also re-render Luneko 08_searching and 03_ai_thinking with perfect_matte
    luneko_master = f"{BASE_DOWN}/mascots/luneko/master/luneko_greenscreen_master.jpg"
    luneko_search_prompt = "Starts from the neutral resting stance on solid chroma green (#00FF00). The cute 3D porcelain ginger cat Luneko holds a stylized 3D magnifying glass with an orange ceramic rim, round handle, and transparent glass lens in its right paw, looking through the lens from left to right with curiosity, smooth 3D character animation."
    render_and_process("luneko", "08_searching", luneko_search_prompt, luneko_master)
    
    luneko_think_prompt = "Starts from the neutral resting stance on solid chroma green (#00FF00). The cute 3D porcelain ginger cat Luneko tilts its head to the side and rests its right paw thoughtfully under its chin in deep contemplation, looking slightly upward in focus, smooth 3D character animation."
    render_and_process("luneko", "03_ai_thinking", luneko_think_prompt, luneko_master)
    
    print("\n🎉 HARMONISATION FINALE PARFAITE TERMINÉE !")
