#!/usr/bin/env python3
"""
Unit test suite verifying the visual and mathematical integrity of the Owluko Master Turnaround deliverables.
Covers:
...
5. Strictly ZERO hands, ZERO fingers, ZERO thumbs (smooth teardrop winglets and 3-toed porcelain feet only).
...
OWLUKO_DIR = os.path.join(WORKSPACE_DIR, "mascots/owluko")
REFONTE_DIR = os.path.join(OWLUKO_DIR, "02_refonte_nouvelle/master_turnaround")

class TestOwlukoTurnaroundVisualIntegrity(unittest.TestCase):

    def test_01_all_deliverables_exist_and_non_empty(self):
        """Verify all 9 canonical deliverables exist and have non-zero size."""
        required_files = [
            "owluko_master_turnaround_sheet.png",
            "owluko_studio_turnaround_sheet.png",
            "owluko_turnaround_master_board.png",
            "owluko_master_side_by_side.png",
            "owluko_master_exact_512.png",
            "owluko_master_turnaround.svg",
            "owluko_3d_turnaround_viewer.html",
        ...
        for fname in required_files:
            # Check in main owluko dir
            p1 = os.path.join(OWLUKO_DIR, fname)
            self.assertTrue(os.path.exists(p1), f"Missing file: {p1}")
    ...
    def test_02_canvas_dimensions(self):
        ...
        specs = {
            "owluko_master_turnaround_sheet.png": (1920, 820),
            "owluko_studio_turnaround_sheet.png": (1920, 820),
            "owluko_turnaround_master_board.png": (1920, 1600),
            "owluko_master_side_by_side.png": (1024, 512),
            "owluko_master_exact_512.png": (512, 512),
            "owluko_turnaround_view_1_front.png": (512, 512),
            "owluko_turnaround_view_2_profile_right.png": (512, 512),
            "owluko_turnaround_view_3_profile_left.png": (512, 512),
            "owluko_turnaround_view_4_back.png": (512, 512)
        }

        for fname, expected_dim in specs.items():
            p = os.path.join(OWLUKO_DIR, fname)
    ...
    def test_03_porcelain_color_fidelity_and_no_green_spill(self):
        ...
        front_p = os.path.join(OWLUKO_DIR, "owluko_turnaround_view_1_front.png")
        with Image.open(front_p) as img:
            arr = np.array(img)
            r, g, b, a = arr[:, :, 0], arr[:, :, 1], arr[:, :, 2], arr[:, :, 3]

            # Sample porcelain belly / forehead (high alpha, non-eye region)
    ...
    def test_04_amber_eye_chromaticity(self):
        ...
        with Image.open(front_p) as img:
            ...
            # Eye bounding zone (upper third of mascot)
            ...
            self.assertLess(np.mean(b_eye), 75.0, "Amber eye blue component is too high (lacks amber warmth)")
    ...
    def test_05_strictly_zero_human_hands_or_fingers(self):
        ...
        # Check SVG
        svg_p = os.path.join(OWLUKO_DIR, "owluko_master_turnaround.svg")
        ...
        script_p = os.path.join(WORKSPACE_DIR, "scripts/build_flawless_owluko_turnaround_master.py")
        with open(script_p, "r", encoding="utf-8") as f:
            script_text = f.read().upper()
            self.assertIn("ZÉRO DOIGTS / ZÉRO MAINS", script_text)
    ...
    def test_07_interactive_viewer_html_structure(self):
        ...
        viewer_p = os.path.join(OWLUKO_DIR, "owluko_3d_turnaround_viewer.html")
        with open(viewer_p, "r", encoding="utf-8") as f:
            html = f.read()
            self.assertIn("owluko_turnaround_view_1_front.png", html)
            self.assertIn("owluko_turnaround_view_2_profile_right.png", html)
            self.assertIn("owluko_turnaround_view_3_profile_left.png", html)
            self.assertIn("owluko_turnaround_view_4_back.png", html)
